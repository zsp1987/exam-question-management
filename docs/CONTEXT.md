...
- **编辑器**：TipTap + KaTeX + 8×8 网格表格生成器（悬停拖拽选定尺寸，支持 +Row/+Col 动态增删；硬上限 20×20；禁止合并单元格与嵌套表格；单元格可含 LaTeX）。
- **公式定界**：`$$...$$` 块级 + `$...$` 行内（仅字母边界触发），`$750` 等货币文本不触发；`$R×C$` 类建议用 `$$...$$`。
- **净化（Defense in Depth）**：写入时 + 渲染/导出时双重 sanitize（DOMPurify/sanitize-html）；allow-list：`table/thead/tbody/tr/th/td`, `span.katex`, `code/pre`, `ul/ol/li`, `a[href]`；一律剥离 `<script>/<iframe>/<style>` 与 `on*` 事件属性。
