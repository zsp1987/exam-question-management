...
### 3.2 题库大厅与富文本/公式/表格编辑器（WYSIWYG TipTap）
- **题型支持**: 单选题 (Single Choice)、多选题 (Multiple Choice)、场景综合/问答题 (Scenario / Essay)。
- **WYSIWYG 编辑器（TipTap，Q1 a / Q2 a）**：真 `contentEditable` 所见即所得，覆盖 `stem_rich_text` + `options_json[].text` + `standard_answer_rich_text` + `explanation_rich_text` 全部 4 处；存储为 sanitized HTML（`editor.getHTML()` → `sanitizeHtml()`），`katex_source` 由 `data-latex` 汇总；存量 HTML 兼容；单字段 50KB 上限。
  - **工具栏（Q5 a，每编辑器固定一行，4 处完全一致）**：`B I | Table ▾ | fx Formula | H2 | List | Code`（位于编辑区上方）+ 表格内气泡 `+Row/−Row/+Col/−Col/Toggle Header/Delete`；i18n 经 `I18nContext`；无 slash 命令。
  - **表格（Q3 a）**：8×8 悬停网格插入真实 TipTap table 节点；气泡动态增删行列；硬上限 20×20（达限禁用）；禁止合并/嵌套；单元格可含 LaTeX。
  - **公式（Q4 a）**：Add Formula 弹窗 — LaTeX 输入 + 实时 KaTeX 预览 + Inline/Block 切换 + 符号面板（复用 FormulaToolbar 分类）；插入 atom 节点 `inlineMath`(`span.katex-inline`)/`blockMath`(`div.katex-block`)；`$750` 字母边界规则在粘贴时强制。
- **公式定界**：块级 `$ ... $` + 行内 `$ ... # Functional Specification: Professional Certification EQMS
