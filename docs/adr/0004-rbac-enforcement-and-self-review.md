...
## Decision
1. **权限矩阵**：见 SPEC §2.2 可执行矩阵；VIEWER 仅见 APPROVED（含导出/KPI）；TEACHER 仅见本人 DRAFT/PENDING/REJECTED + 全部 APPROVED；ADMIN/REVIEWER 见全部。
...
7. **图片**：jpg/png/webp，≤5MB，TEACHER/ADMIN/REVIEWER 可上传；磁盘 `uploads/<uuid>.*` + URL 引用；禁用 Base64 内嵌与 SVG；导出捆绑 + manifest.json。
8. **i18n**：仅 UI chrome 翻译，内容保持原文；可选 content_locale 过滤。
9. **难度**：1-5 主观分级，TEACHER 建议、REVIEWER 终审，仅报表用途。
