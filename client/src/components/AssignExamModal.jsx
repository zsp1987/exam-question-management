...
export default function AssignExamModal({ question, onClose, onSuccess }) {
  ...
  const [domainSection, setDomainSection] = useState('Domain 1: Core Architecture & Knowledge');
  ...
  const handleAssign = async (e) => {
    ...
    try {
      setSubmitting(true);
      await api.addQuestionsToExam(selectedExamId, [question.id], domainSection, 1.0);
      alert(lang === 'en' ? 'Question successfully assigned to certification exam folder!' : '已成功归入指定认证考试文件夹！');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="mb-4 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs">
          <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Target Question:</span>
          <p className="font-bold text-slate-800"><MathRenderer content={question.title} /></p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-6 text-center text-xs text-slate-400">
            {lang === 'en' ? 'No exam folders available. Please create an exam folder first.' : '暂无认证考试文件夹，请先在“认证考试文件夹”页面创建。'}
          </div>
        ) : (
          <form onSubmit={handleAssign} className="space-y-4 text-xs">
            <div>
              <label className="block font-bold text-slate-700 mb-1">
                {lang === 'en' ? 'Select Certification Exam' : '选择目标认证考试'} *
              </label>
              <select
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">
                {lang === 'en' ? 'Exam Domain / Module Section' : '考纲对应知识模块'}
              </label>
              <input
                type="text"
                value={domainSection}
