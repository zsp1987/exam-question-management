...
export const api = {
  ...
  switchUser: (userId) => request('/auth/switch-user', {
    method: 'POST',
  ...
  addQuestionsToExam: (id, questionIds, domain_section, score_weight) => request(`/exams/${id}/questions`, {
    method: 'POST',
    body: JSON.stringify({ questionIds, domain_section, score_weight })
  }),
  removeQuestionFromExam: (id, questionId) => request(`/exams/${id}/questions/${questionId}`, {
    method: 'DELETE'
  }),
  exportExam: (id, format = 'markdown') => {
    if (format === 'markdown') {
      return request(`/exams/${id}/export?format=markdown`, { responseType: 'text' });
    }
  ...
  submitForReview: (id, comment) => request(`/questions/${id}/submit-review`, {
    method: 'POST',
    body: JSON.stringify({ comment })
  }),
  getVersions: (id) => request(`/questions/${id}/versions`),
  getVersion: (id, versionId) => request(`/questions/${id}/versions/${versionId}`),
  rollbackVersion: (id, versionId) => request(`/questions/${id}/rollback/${versionId}`, {
    method: 'POST'
  }),
  ...
  submitReviewDecision: (id, action, comment) => request(`/reviews/${id}/decision`, {
    method: 'POST',
  ...
  exportQuestions: (format = 'json', params = {}) => {
    ...
    if (format === 'markdown') {
      return request(`/stats/export?${query.toString()}`, { responseType: 'text' });
    }
  ...
  createAdminUser: (data) => request('/admin/users', {
    method: 'POST',
    body: JSON.stringify(data)
  }),
  updateAdminUser: (id, data) => request(`/admin/users/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data)
  }),
