...
router.put('/profile', authenticateToken, (req, res) => {
  ...
  if (email && email.trim() && email.trim() !== existing.email) {
    const dup = db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(email.trim(), userId);
  ...
  db.prepare('UPDATE users SET name = ?, email = ?, password_hash = ? WHERE id = ?').run(newName, newEmail, newHash, userId);
  logAudit(userId, existing.username, 'UPDATE_PROFILE', 'USER', userId, `Updated profile: ${newName} <${newEmail}>`);

  const updated = db.prepare('SELECT id, username, email, role, name, avatar, created_at FROM users WHERE id = ?').get(userId);
  // Issue new token so name in token stays fresh (optional)
  const token = jwt.sign({ id: updated.id, username: updated.username, role: updated.role, name: updated.name }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/avatar', authenticateToken, (req, res) => {
  avatarUpload.single('avatar')(req, res, (err) => {
    ...
    if (!req.file) return res.status(400).json({ error: 'No avatar file received' });
    ...
    logAudit(req.user.id, req.user.username, 'UPLOAD_AVATAR', 'USER', req.user.id, `Uploaded avatar -> ${url}`);
    const updated = db.prepare('SELECT id, username, email, role, name, avatar, created_at FROM users WHERE id = ?').get(req.user.id);
    const token = jwt.sign({ id: updated.id, username: updated.username, role: updated.role, name: updated.name }, JWT_SECRET, { expiresIn: '7d' });
