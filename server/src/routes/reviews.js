...
router.get('/pending', authenticateToken, requireRole(['REVIEWER', 'ADMIN']), (req, res) => {
  ...
  const where = ["q.status = 'PENDING_REVIEW'", "q.deleted_at IS NULL"];
  const params = [];
  ...
  const sql = `
    SELECT 
      ...
      qv.standard_answer_rich_text, qv.explanation_rich_text, qv.katex_source, qv.change_summary,
      u.name as author_name, u.username as author_username
    FROM questions q JOIN question_versions qv ON q.current_version_id = qv.id
    LEFT JOIN users u ON q.author_id = u.id
    WHERE ${where.join(' AND ')} ORDER BY q.updated_at ASC
  `;
  const rows = db.prepare(sql).all(...params);
  const questions = rows.map(q => {
    let options = []; if (q.options_json) try { options = JSON.parse(q.options_json); } catch (e) {}
    const tags = db.prepare(`SELECT t.id, t.name, t.category, t.color FROM tags t JOIN question_tags qt ON t.id = qt.tag_id WHERE qt.question_id = ?`).all(q.id);
...
router.post('/:id/decision', authenticateToken, requireRole(['REVIEWER', 'ADMIN']), (req, res) => {
  ...
  if (!action || !['APPROVE', 'REJECT'].includes(action)) return res.status(400).json({ error: '审核动作必须为 APPROVE (通过) 或 REJECT (驳回)' });
  ...
  if (question.status !== 'PENDING_REVIEW') return res.status(400).json({ error: `该考题当前状态为 [${question.status}]，无法执行审核操作` });
  const newStatus = action === 'APPROVE' ? 'APPROVED' : 'REJECTED';
  const reviewId = uuidv4();
  const transaction = db.transaction(() => {
    db.prepare(`UPDATE questions SET status = ?, reviewer_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(newStatus, req.user.id, id);
    db.prepare(`INSERT INTO review_records (id, question_id, version_id, reviewer_id, action, comment) VALUES (?, ?, ?, ?, ?, ?)`).run(reviewId, id, question.current_version_id, req.user.id, action, comment.trim());
  });
  try {
    transaction();
    ...
    res.json({ success: true, message: `考题已${actionText}`, status: newStatus });
  } catch (err) {
...
router.get('/records', authenticateToken, (req, res) => {
  ...
  const sql = `
    SELECT r.id, r.question_id, r.action, r.comment, r.created_at,
